...
function commands() {
  return [
    ...
    // ---------------------------------------------------------- appearance
    { key: "theme.pick",  title: "Change Theme",             subtitle: "Open the theme switcher",     icon: "󰸌", kind: "shell", argv: ["bash", "-c", "theme=$(omarchy-theme-switcher); [[ -n $theme ]] && omarchy-theme-set \"$theme\""],       keywords: "theme colors colours appearance style dark light" },
    { key: "theme.bg",    title: "Change Background",        subtitle: "Open the wallpaper picker",   icon: "󰸉", kind: "shell", argv: ["bash", "-c", "bg=$(omarchy-theme-bg-switcher); [[ -n $bg ]] && omarchy-theme-bg-set \"$bg\""],    keywords: "wallpaper background image desktop theme" },
    ...
    { key: "ses.lock",    title: "Lock Screen",              subtitle: "Lock and blank the display",  icon: "󰌾", kind: "shell", argv: ["omarchy", "system", "lock"],       keywords: "lock screen session secure away system power" },
    { key: "ses.logout",  title: "Log Out",                  subtitle: "End the session",             icon: "󰗽", kind: "shell", argv: ["omarchy", "system", "logout"],     keywords: "logout log out sign out exit session system power", confirm: true },
    { key: "ses.reboot",  title: "Restart",                  subtitle: "Reboot the machine",          icon: "󰜉", kind: "shell", argv: ["omarchy", "system", "reboot"],     keywords: "reboot restart system power", confirm: true },
    { key: "ses.off",     title: "Shut Down",                subtitle: "Power off",                   icon: "󰐥", kind: "shell", argv: ["omarchy", "system", "shutdown"],   keywords: "shutdown power off halt system", confirm: true }
  ]
}
